...
uint8_t StaticRingBuf_Init(StaticRingBuf* rbuf, const STARB_STOSZTP total_buffer_size, byte* _buffer)
{
    ...
    if (rbuf == NULL || _buffer == NULL)
    {
        rc = STARB_PARAM_NULL;
        goto EXIT;
    }

    if (total_buffer_size < 2 || total_buffer_size > STARB_MAXCAP)
    {
    ...
    rbuf->buffer = _buffer;
    rbuf->capacity = (STARB_CAPTYPE)(total_buffer_size / 2);
    rbuf->wpos = 0;
    rbuf->rpos = 0;
    rbuf->flag.zeros = 0;
    rbuf->flag.cycle = 0;
...
uint8_t StaticRingBuf_Write(StaticRingBuf* rbuf, const byte _elem)
{
    ...
    size_t mirror_offset = (size_t)rbuf->wpos + (size_t)rbuf->capacity;
    rbuf->buffer[mirror_offset] = _elem;
...
uint8_t StaticRingBuf_Forward(StaticRingBuf* rbuf, const STARB_CAPTYPE skipcount)
{
    ...
    STARB_CAPTYPE read_capacity = StaticRingBuf_GetReadCapacity(rbuf);
    if (read_capacity <= 0 || skipcount > read_capacity)
    {
        rc = STARB_NOENOUGHDAT;
        goto EXIT;
    }

    if (rbuf->rpos >= rbuf->capacity - skipcount)
    {
        if (rbuf->flag.cycle == 0)
        {
            rc = STARB_DATAINVALID;
        ...
        {
            rbuf->rpos -= (rbuf->capacity - skipcount);
            rbuf->flag.cycle = 0;
        }
    }
    else
